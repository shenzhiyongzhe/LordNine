  ...
  let Api_调用瑞科接口类 = function () {
    ...
    this.GetRequestResult = function (业务数据_Json对象, 业务类型) {
      ...
      for (let i = 0; i < repNum; i++) {
        result = this.getReqResult(Rkyz属性类.接口请求地址, ApiArgs);
        if (业务类型 == Rkyz属性类.业务类型.登录成功后心跳) {
          if (result.code == -999) {
            break;
          }
          if (result.code == 1001 || result.code == 1002 || result.code == 1012) {
            result.code = 0;
            result.data = null;
            break;
          }
        ...
        if (result.code == -999) {
          输出日志("切换中2...");
          result = this.getReqResult("http://api2.ruikeyz.com/NetVer/webapi", ApiArgs);
        }
        if (result.code == -999) {
          输出日志("切换中3...");
          result = this.getReqResult("http://api3.ruikeyz.com/NetVer/webapi", ApiArgs);
        }
        if (result.code != -999) {
          break;
        }

        sleep(500 * sleepReqTime);
      }
    ...
    this.getReqResult = function (reqAddres, ApiArgs) {
      let result = new Result();
      result.code = -999;
      result.msg = "服务器返回失败";
      try {
        if (reqAddres == Rkyz属性类.接口请求地址) {
          http.__okhttp__.setTimeout(6000);
        }
        else {
          http.__okhttp__.setTimeout(10000);
        }

        let ResponseResult = http.postJson(reqAddres, ApiArgs,
          {
            "headers": {
    ...
    ui.layout(
      <vertical h="*" bg="#FFFFFF">
        ...
        <vertical layout_weight="1" >
          <horizontal >
            <text w="35" ellipsize="middle" maxLines="1" text="卡密:" size="14" color="#000000" marginLeft="10" marginTop="20"></text>
            <input id="cardTxt" h="50" hint="请输入卡密" gravity="center" layout_weight="1" />
          </horizontal>
          <horizontal >
              ...
              let threadHeartbeat = threads.start(function () {
                ...
                while (true) {
                  ...
                  if (result_Hearbeat.code != 0) {
                    ...
                    errorNum = 0;
                    try {
                      if (result_Hearbeat != null && result_Hearbeat.data != null) {
                ...
                if (isExec) {
                  ...
                  try {
                    let dialogsValue = dialogs.build({
                      title: "提示信息",
                      content: "错误编码：" + result_Hearbeat.code + "   错误消息：" + result_Hearbeat.msg,
                      positive: "确定",
                      cancelable: false
                    }).on("positive", () => {
                      rkyzSDK.强制关闭本应用();
